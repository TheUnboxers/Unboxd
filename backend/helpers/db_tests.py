...
class DBTests(unittest.TestCase):
    ...
    # Every test method needs to be prefixed with `test` for unittest to detect it
    def test_get_letterboxd_user_id(self):
        """
    ...
    def test_get_expiration_timestamp(self):
        ...
        self.assertGreaterEqual(base_expiration_timestamp, current_timestamp)
        self.assertLessEqual(base_expiration_timestamp, timestamp_after_base_ttl)
        ...
        self.assertGreaterEqual(proportional_expiration_timestamp, current_timestamp)
        self.assertLessEqual(proportional_expiration_timestamp, timestamp_after_ttl)
    ...
    def test_has_expired(self):
        ...
        self.assertTrue(has_expired(Recommendation(expiration_timestamp=expired_timestamp)))
        self.assertFalse(has_expired(Recommendation(expiration_timestamp=not_expired_timestamp)))


    def test_delete_expired_recommendations(self):
        """
        ...
        with Session(engine) as session:
            ...
            self.assertEqual(len(all_recommendations), 1)        
            

    def test_get_features_and_ratings(self):
        """
    ...
    def test_extract_imdb_ids_from_recommendation(self):
        ...
        with Session(engine) as session:
            ...
            self.assertEqual(len(imdb_id_kwargs), len(extracted_imdb_ids))
            self.assertEqual(len(set(imdb_id_kwargs.values()) - set(extracted_imdb_ids)), 0)
    ...
    def test_cache_recommendation(self):
        ...
        with Session(engine) as session:
            ...
            if recommendation is not None:
                ...
                self.assertEqual(len(imdb_ids), len(rec_imdb_ids))
                self.assertEqual(len(set(imdb_ids) - set(rec_imdb_ids)), 0)
    ...
    def test_get_movies(self):
        ...
        with Session(engine) as session:
            ...
            self.assertEqual(len(imdb_ids), len(movie_imdb_ids))
            self.assertEqual(len(set(imdb_ids) - set(movie_imdb_ids)), 0)
    ...
    def test_cache_trailer_ids(self):
        ...
        with Session(engine) as session:
            ...
            self.assertEqual(len(trailer_ids), len(stored_trailer_ids))
            self.assertEqual(len(set(trailer_ids) - set(stored_trailer_ids)), 0)
