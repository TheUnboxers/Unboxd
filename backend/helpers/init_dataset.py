...
def read_csv_to_df(info: tuple[str, dict[Hashable, Callable]]) -> pd.DataFrame:
    """
    Reads the csv file at `path`, applies `converters`, and does not create an index.

    Args:
        `info`: (`path`, `converters`) where `path` points to a csv and
                `converters`, a mapping of column names to converter functions,
                is used to process columns of data when loading them into a `DataFrame`.

    Returns:
        The `DataFrame` resulting from reading `path` and applying `converters` to it.
    """
...
def read_csvs_to_df(csv_dir: str, cols_to_load: Sequence[Hashable] = []) -> pd.DataFrame:
    ...
    Returns:
        The `DataFrame` resulting from reading the csvs in `csv_dir`, and loading `cols_to_load`.
    """
...
def main():
    ...
    df.info()
    print(
        "Note: the empty list values of features like keywords do not appear as null"
    )
